...
function otraUserInitDatabase(array $argumentsVector): void
{
  ...
  if (isset($argumentsVector[OTRA_USER_INIT_ARG_DB_CONNECTION_KEY]))
  {
    if (!isset(AllConfig::$dbConnections[$argumentsVector[OTRA_USER_INIT_ARG_DB_CONNECTION_KEY]]))
    {
      echo CLI_ERROR, 'There is no database connection key ', CLI_INFO_HIGHLIGHT,
      $argumentsVector[OTRA_USER_INIT_ARG_DB_CONNECTION_KEY], CLI_ERROR, ' in your ', CLI_INFO_HIGHLIGHT, 'dbConnections',
      CLI_ERROR, ' configuration parameter!', END_COLOR, PHP_EOL;
      throw new OtraException(code:1, exit: true);
    }

    define(__NAMESPACE__ . '\\DB', AllConfig::$dbConnections[$argumentsVector[OTRA_USER_INIT_ARG_DB_CONNECTION_KEY]]['db']);
    $db = Sql::getDb($argumentsVector[OTRA_USER_INIT_ARG_DB_CONNECTION_KEY], false);
  } else
  {
  ...
  define(
    __NAMESPACE__ . '\\NAME',
    isset($argumentsVector[OTRA_USER_INIT_ARG_NAME])
      ? intval($argumentsVector[OTRA_USER_INIT_ARG_NAME])
      : 0
  );

  define(
    __NAMESPACE__ . '\\ROLES',
    isset($argumentsVector[OTRA_USER_INIT_ARG_ROLES])
      ? intval($argumentsVector[OTRA_USER_INIT_ARG_ROLES])
      : 0
  );
